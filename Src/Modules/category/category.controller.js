...
export const addCatecory = async (req, res, next) => {
  ...
  if (isNameDuplicated) {
    return next({ message: "Name already exist", cause: 409 });
  }
  // 3- generate the slug
...
export const getAllCategories = async (req, res, net) => {
  // nested populate
...
//====================== Get all Sub-category for specific category ======================//

export const getAllSubCatecoryForSpecificCategory =async(req,res,next)=>{
  const {categoryId} = req.params
  const subCategories = await subCategory.find({categoryId:categoryId})
  if(!subCategories.length){
    return next(new Error('Not found sub-categories for this category'))
  }
  res.status(200).json({message:"Success",subCategories})
}
export const getCtegoryById =async(req,res,next)=>{
  const {categoryId} = req.params
  const category = await Category.findById(categoryId)
  if(!category){
